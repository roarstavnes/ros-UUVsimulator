...
bool IMUROSPlugin::OnUpdate(const common::UpdateInfo& _info)
{
  ...
  // Compute the simulated measurements wrt the NED frame
  ignition::math::Vector3d gravityB(-gravityBody.Y(), - gravityBody.X(), -gravityBody.Z()); // Change is made here

  this->measLinearAcc = bodyLinAcc - gravityB;
